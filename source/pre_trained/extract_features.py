...
def normalize_vector(data):
	data_max = pickle.load( open( "max_values.p", "rb" ) )
	data_min = pickle.load( open( "min_values.p", "rb" ) )
	temp_data = []
	for i in range(0,len(data)):
		temp_data.append((float(data[i]) - data_min[i])/(data_max[i]-data_min[i]))
	return temp_data
